 ...
 * 工具的电源类型
 **/
typedef enum { OUT_0V = 0, OUT_12V = 1, OUT_24V = 2 } ToolPowerType;
...
/** 机械臂状态枚举　**/
enum RobotState { RobotStopped = 0, RobotRunning, RobotPaused, RobotResumed };

/** 运动模式枚举　**/
enum move_mode { NO_MOVEMODE = 0, MODEJ, MODEL, MODEP };
...
/** 坐标系枚举　**/
enum coordinate_refer { BaseCoordinate = 0, EndCoordinate, WorldCoordinate };
...
typedef struct {
    coordinate_refer
        coordType;  // 坐标系类型：当coordType==BaseCoordinate或者coordType==EndCoordinate是，下面3个参数不做处理

    CoordCalibrateMathod methods;  // 坐标系标定方法

    JointParam
        wayPointArray[3];  // 用于标定坐标系的３个点（关节角），对应于机械臂法兰盘中心点基于基座标系
 ...
 */
typedef void (*RealTimeJointStatusCallback)(const aubo_robot_namespace::JointStatus *jointStatus,
                                            int size, void *arg);
 ...
 * @param arg　　　　　　这个参数是使用者在注册回调函数中传递的第二个参数;
 */
typedef void (*RealTimeRoadPointCallback)(const aubo_robot_namespace::wayPoint_S *wayPoint,
                                          void *arg);
