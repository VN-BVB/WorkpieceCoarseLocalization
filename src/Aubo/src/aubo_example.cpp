...
#include "../include/aubo_example.h"

#define M_PI 3.14159265358979323846

//#define ROBOT_ADDR "192.168.65.138"
//#define ROBOT_PORT 8899

//#define ROBOT_ADDR "192.168.0.10"
//#define ROBOT_PORT 8899
...
void printRoadPoint(const aubo_robot_namespace::wayPoint_S  *wayPoint)
{
	...
	//四元数转欧拉角
    ////
	double x = wayPoint->orientation.x, y = wayPoint->orientation.y, z = wayPoint->orientation.z, w = wayPoint->orientation.w;
	double roll, pitch, yaw;
	toEulerAngle(x, y, z, w, roll, pitch, yaw);
	std::cout << "rpy_rx=" << roll * 180.0/M_PI << std::endl;
	std::cout << "rpy_ry=" << pitch * 180.0/M_PI << std::endl;
	std::cout << "rpy_rz=" << yaw * 180.0/M_PI << std::endl;
	////
	std::cout << std::endl;
...
bool example_moveL(RSHD rshd)
{
	...
	//目标位置
	Pos pos = {-0.489605, -0.155672, 0.448430};//单位 米

	if (RS_SUCC == rs_get_current_waypoint(rshd, &wayPoint))//获取当前的位置、姿态、关节角信息
	{
		//参考当前姿态逆解得到六个关节角
		if (RS_SUCC == rs_inverse_kin(rshd, wayPoint.jointpos, &pos, &wayPoint.orientation, &targetPoint))//当前关节角；目标位置、目标姿态；逆解目标关节角
		{
			//将得到目标位置,将6关节角度设置为用户给定的角度（必须在+-175度）
			targetRadian[0] = targetPoint.jointpos[0];
			targetRadian[1] = targetPoint.jointpos[1];
...
/********************************************************************
	...
	purpose :	四元数转欧拉角
	param   :	x y z w 四元数 roll pitch yaw 欧拉角
