...
void HandEyeCalibrationLogic::whenCalibrateEye2Hand()
{
    ...
    if (0) {
        ...
        //HandEyeCalibration::calibrateEyeInHand(Hcg, vecHg, vecHc, HandEyeCalibration::HAND_EYE_TSAI, resultPath + "EyeInHandMatrix.yml");//5.6ms
    }
...
void HandEyeCalibrationLogic::getTrackDirection(std::string path, cv::Mat& HcgTrackCalib){//指向原点移动
    ...
    for (int i = 0; i < vecHc.size(); ++i) {
        ...
        for (int j = 0; j < objPoints.size(); ++j) {
            ...
            // 提取机器人基坐标系下的 3D 坐标
            worldPoints.push_back(cv::Point3d(robotBasePoint.at<double>(0),
                                              robotBasePoint.at<double>(1),
                                              robotBasePoint.at<double>(2)));
        }
    ...
    double magnitude = sqrt(totalTranslation.x * totalTranslation.x +
    ...
    cv::Point3d unitDirection(totalTranslation.x / magnitude,
...
void HandEyeCalibrationLogic::test(std::string trackPath)
{
    ...
    // 手动给定的平移向量
    cv::Mat Hc1_manual1 = (cv::Mat_<double>(4, 4) <<
                               0.9995132115610255, -0.02019904319366579, -0.02377684943435908, -0.8605622719022955,
                           -0.02025012754446433, -0.9997931213772339, -0.001909654733043817, -0.1559250071294697,
                           -0.02373335731405568, 0.002390209368848928, -0.9997154668453262, 1.06690330933994,
                           0, 0, 0, 1);

    cv::Mat Hc2_manual1 = (cv::Mat_<double>(4, 4) <<
                               0.9998209637165499, -0.001445359319344509, -0.01886667563052896, -0.7248343721691441,
                           -0.001446082067449214, -0.999998954118583, -2.466563952358003e-05, -0.0560321003531572,
                           -0.01886662024751207, 5.194398478103324e-05, -0.9998220081305766, 1.064627607740112,
                           0, 0, 0, 1);
    cv::Size boardSize = {board_width, board_heignt}; // 标定板规格
...
double ReprojectionErrorCallback::calculate_Reprojection_Error(const std::vector<cv::Point2f>& imagePoints,
    ...
    for (size_t i = 0; i < objectPoints.size(); ++i) {
        ...
        if (std::isnan(objectPoints[i].x) || std::isnan(objectPoints[i].y) || std::isnan(objectPoints[i].z) ||
            ...
            std::cerr << "Warning: Invalid 3D point at index " << i << ": "
                      << "x = " << objectPoints[i].x << ", y = " << objectPoints[i].y << ", z = " << objectPoints[i].z << std::endl;
            // 可以选择抛出异常或者返回某种错误值
            continue ;  // 或者其他错误处理方式
        }
        ...
        cv::projectPoints(point_camera.rowRange(0, 3),
