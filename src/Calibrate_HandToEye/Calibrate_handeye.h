...
public:
    ...
    double calculate_Reprojection_Error(const std::vector<cv::Point2f>& imagePoints,
...
public:
    ReprojectionErrorCallback(const std::vector<std::vector<cv::Point2f>>& _imagePoints,
                              const std::vector<cv::Point3f>& _objPoints,
                              const std::vector<cv::Mat>& _vecHg,
                              const cv::Mat& _Kc,
                              const cv::Mat& _camera_distortion)
        : imagePoints(_imagePoints), objPoints(_objPoints), vecHg(_vecHg),
        Kc(_Kc), camera_distortion(_camera_distortion) {}
    ...
    bool compute(cv::InputArray param, cv::OutputArray err, cv::OutputArray J) const override {
        ...
        for (size_t i = 0; i < imagePoints.size(); i++) {
            totalError += calculate_Reprojection_Error(imagePoints[i], objPoints,
                                                        TCW, vecHg[i], TEB, Kc, camera_distortion);
        ...
        else if( J.empty() ) J.create(/*imagePoints.size()*/1, 24, CV_64F);
        ...
        for (int i = 0; i < 24; ++i) {  // 修改这里的循环范围
            ...
            for (size_t j = 0; j < imagePoints.size(); j++) {
                newError += calculate_Reprojection_Error(imagePoints[j], objPoints,
                                                         TCWEpsilon, vecHg[j], TEBEpsilon,
                                                         Kc, camera_distortion);
            }
    ...
    double calculate_Reprojection_Error(const std::vector<cv::Point2f>& imagePoints,
                                        const std::vector<cv::Point3f>& objectPoints,
                                        const cv::Mat& TCW, const cv::Mat& TWE,
                                        const cv::Mat& TEB, const cv::Mat& K,
...
private:
    ...
    cv::Mat refValues = (cv::Mat_<double>(12, 1) <<
                             -0.001720457820086829, -0.9999436738515231, -0.01047325876737607, 0.2442762011199959,
                         -0.9998644588949632, 0.001548644461831869, 0.01639102007239512, -0.7893348578291108,
