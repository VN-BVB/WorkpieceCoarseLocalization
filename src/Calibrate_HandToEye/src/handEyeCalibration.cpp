#include "src/Calibrate_HandToEye/include/handEyeCalibration.h"
...
*
*/
bool HandEyeCalibration::readRobotDatasFromFile(
	const std::string & folder,
	std::vector<cv::Mat>& vecHg, bool useQuaternion)
{
	//位姿大矩阵
	cv::Mat attitudeBigMat;
	std::vector<cv::String> xmlFiles;
	...
	for (int i = 0; i < xmlFiles.size(); ++i)
	{
		cv::FileStorage fs(xmlFiles[i], cv::FileStorage::READ);
		if (!fs.isOpened()) {
			std::cout << "Can not open \"" << xmlFiles[i] << "\"." << std::endl;	return false;
		}
...
cv::Mat HandEyeCalibration::eulerAngleToRotatedMatrix(const cv::Mat& eulerAngle, const std::string& seq)
{
	...
	else if (seq == "zyz")	rotMat = rotZ * rotY*rotZ;  //新添加
	else {
