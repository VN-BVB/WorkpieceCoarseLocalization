...
#include "src/Calibrate_HandToEye/include/others.h"
...
void calc_stdev(vector<double> &vecWorld, double &stdev, double &mean)
{
	...
	double accum = 0.0;
	std::for_each(std::begin(vecWorld), std::end(vecWorld), [&](const double d) {
		accum += (d - mean) * (d - mean);
	});
...
bool calculate_Image_Points_ChessboardCorners(const std::string &path, cv::Size boardSize, std::vector<std::vector<cv::Point2f>> &imagePoints) {
...
//vector
bool calculate_Image_Points_V(std::vector<cv::Mat> &dirImages, cv::Size boardSize, std::vector<std::vector<cv::Point2f>> &imagePoints)
{
    ...
    for (int i = 0; i < nframes; i++)
    {
        ...
        bool found;
        if(1){
            found = findCirclesGrid(viewGray, boardSize, pointbuf, cv::CALIB_CB_SYMMETRIC_GRID | cv::CALIB_CB_CLUSTERING, blobDetector);
        }else{
            found = findChessboardCornersSB(viewGray, boardSize, pointbuf, cv::CALIB_CB_ADAPTIVE_THRESH | cv::CALIB_CB_NORMALIZE_IMAGE);
        }
        if (found)
        {
            if (pointbuf[0].x > pointbuf[pointbuf.size() - 1].x) {
...
void draw_line_chart_visualization(pcl::visualization::PCLPlotter *plot_, vector<double> &X_data, vector<double> &Y_data, string string_title)
{
	const char *title = string_title.c_str();
