...
void CalibratateCamera::whenNeedCalibratateCamera()
{
    ...
    // 输出所有找到的图片文件路径
    if (!imagePaths.empty()) {
        CameraAndLaserPlaneCalibration::cameraCalibration(imagePaths, cameraMatrix,
                                                          distCoeffs, tvecsMat, rvecsMat,
                                                          imageCount,totalErr);
        ...
        CameraAndLaserPlaneCalibration::planeCalibration(imagePlanePaths,cameraMatrix,
                                                         distCoeffs, globalPlane,
                                                         errorMetrics);
        QString errorLog = QString("相机标定误差: %1 mm\n"
                                   "平面拟合误差:\nX: %2 mm\nY: %3 mm\nZ: %4 mm")
                               .arg(totalErr, 0, 'f', 6)
 ...
 * 并发送信号通知其他组件。
 */
void CalibratateCamera::whenSaveConfigJson(bool cameraIntrinsicParaEnable,
                                           bool extrinsicMatrixEnable){
    ...
    if (cameraConfigMap.find(cameraName) != cameraConfigMap.end()) {
        ...
        if(cameraIntrinsicParaEnable){
            cameraConfig.cameraMatrixData = std::vector<double>(cameraMatrix.begin<double>(),
                                                                cameraMatrix.end<double>());
            cameraConfig.distCoeffsData = std::vector<double>(distCoeffs.begin<double>(),
                                                              distCoeffs.end<double>());
            cameraConfig.globalPlaneData = globalPlane;  // 更新平面数据

        }else if (extrinsicMatrixEnable){
            cameraConfig.extrinsicMatrixData =std::vector<double>(extrinsicMatrix.begin<double>(),
                                                                   extrinsicMatrix.end<double>());
        }
    }else{
        //添加
