        ...
        if (imageSaverToInfer > 0) {
            ...
            for (int j = 0 ; j < calibCameraIndex ; j++){
                ...
                if (ptrGrabResult->GrabSucceeded()) {
                    ...
                    cv::Mat cvImage =
                        cv::Mat(ptrGrabResult->GetHeight(), ptrGrabResult->GetWidth(), CV_8UC3,
        ...
        if(isImageCaptured){
            ...
            try {
                ...
                if (imageNumberToSaveInCalibration > 0) {
                    ...
                    case 0: saveTypePath = "img";
                        savedImages = &savedCalibImages;
                        break;
                    case 1: saveTypePath = "plane";
                        savedImages = &savedPlaneImages;break;
                    case 2: saveTypePath = "compare";
                        savedImages = &savedTrackImages;break;
                    default: saveTypePath = "normal";
                        savedImages = &savednNormalImages;break;
                    }
                    if(saveTypeEnable == 0){
                    ...
                    if ((*savedImages) < 10) {
                        cv::imwrite("./data/calib/camera" + std::to_string(cameraIndex)+"/"+saveTypePath+"/image0" + std::to_string((*savedImages)) + ".bmp", cvImage);
                    } else {
                        cv::imwrite("./data/calib/camera" + std::to_string(cameraIndex)+"/"+saveTypePath+"/image" + std::to_string((*savedImages)) + ".bmp", cvImage);
                    }

                    //std::cout << "camera"+std::to_string(cameraIndex)+"Save image" + std::to_string(savedImages) + " succ." << std::endl;
