...
YOLO11_segCoarse::~YOLO11_segCoarse() {
#ifdef TRT_10
    delete this->context;
    delete this->engine;
    delete this->runtime;
...
void YOLO11_segCoarse::make_pipe(bool warmup) {
...
void YOLO11_segCoarse::letterbox(const cv::Mat &image, cv::Mat &out, cv::Size &size) {
...
void YOLO11_segCoarse::copy_from_Mat(const cv::Mat &image) {
    cv::Mat nchw;
...
void YOLO11_segCoarse::copy_from_Mat(const cv::Mat &image, cv::Size &size) {
    cv::Mat nchw;
    this->letterbox(image, nchw, size);
    // std::cout<< "Letterbox output size: " << nchw.size() << " channels: " << nchw.channels() << " type: " << nchw.type();
    // if (nchw.channels() == 1) {
...
void YOLO11_segCoarse::postprocess(std::vector<seg::Object> &objs, float score_thres, float iou_thres, int topk, int seg_channels,
                                   int seg_h, int seg_w) {
    objs.clear();
    ...
    if (masks.empty()) {
        ...
        // if (masks.cols * masks.channels() % (seg_h * seg_w) != 0) {
        //     std::cerr << "Error: Cannot reshape matrix. Total elements: "
        //               << masks.cols * masks.channels()
        //               << ", Target shape: " << seg_h * seg_w << std::endl;
        //     return;
        // }
        // std::cout << "masks shape: " << masks.rows << " x " << masks.cols << " x " << masks.channels() << std::endl;
        // std::cout << "protos shape: " << protos.rows << " x " << protos.cols << " x " << protos.channels() << std::endl;
        cv::Mat matmulRes = (masks * protos).t();

        // std::cout << "matmulRes total elements: " << matmulRes.total() << std::endl;
        // std::cout << "Expected elements for reshape: " << indices.size() * seg_h * seg_w << std::endl;
    ...
    // std::cout << "Protos shape: " << protos.rows << " x " << protos.cols << std::endl;
}
...
void YOLO11_segCoarse::draw_objects(const cv::Mat &image, cv::Mat &res, const std::vector<seg::Object> &objs,
    ...
    static int imgNum = 0;
    cv::imwrite("./data/result/mask/mask" + std::to_string(imgNum++) + ".bmp", maskOnly);
